  ...
  const renderStars = (note) => {
    ...
    return [...Array(5)].map((_, i) => (
      <button
        ...
        onClick={() => setNote(i + 1)}
      >
        <span className={i < normalizedNote ? "text-warning" : "text-muted"}>
          ★
        </span>
      </button>
    ));
          ...
          <div className="formation-card">
            ...
            <div className="mt-4 border-top pt-3">
              ...
              <div className="formation-card mb-4">
                <h5>Note moyenne</h5>
                <div className="d-flex align-items-center">
                  <div className="d-flex">{renderStars(Math.round(moyenne))}</div>
                  <span className="ms-3 fs-4 fw-bold">{moyenne.toFixed(1)}/5</span>
                </div>
                <small className="text-muted">
                  {evaluationsCount} {evaluationsCount === 1 ? 'évaluation' : 'évaluations'}
                    ...
                    <div className="d-flex align-items-center">
                      {[1,2,3,4,5].map(star => (
                        <button
                          key={star}
                          type="button"
                          className="star-btn me-2"
                          onClick={() => setNote(star)}
                        >
                          <span className={star <= note ? "text-warning" : "text-muted"}>★</span>
                        </button>
                      ))}
                      <span className="ms-2 fw-bold">{note}/5</span>
                    </div>
